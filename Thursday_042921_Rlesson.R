...
for (i in 10:20) { # for loop
  squares[i] = i^2
}
squares


# An alternative to for-loops in R is using the 'apply' family, 
...
# There are several apply functions that vary in the output the return and vary somewhat in the input they require.
# We'll go over sapply "simplifying" apply which returns a vector, first.
...
# calculate the mean for each column using apply
...
apply(states, 2, FUN=funcition(i) ggplot(states, aes(x = population, y = i))+geom_point()+geom_smooth(method="lm"))
 ...
 # We can do the same things across all rows. But if you want to plot all the rows as we did the columns above, 
# I suggest you do that with a smaller dataset than the states dataframe.

#calculate the sum across each row in state
...
# state.region contains the region information for each state
